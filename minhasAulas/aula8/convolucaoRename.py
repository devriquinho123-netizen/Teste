...
def aplicaErosao(mascara, pretoBranco, quant):
    ...
    for q in range(quant):
        for i in range(pretoBranco.shape[0]-2):
            #filtro.append([0]*tomCinza.shape[1])
            for j in range(pretoBranco.shape[1]-2):
                hit=True 
                for k in range(3):
                    for l in range(3):
                        if(q==0):
                            if(k==1 or l ==1):
                                hit= hit and  not(pretoBranco[i+k][j+l])  and  not(mascara[k][l])
                        else:
                            if(k==1 or l ==1):
                                hit= hit and  not(resultado[i+k][j+l])  and  not(mascara[k][l])
                if(hit):
                    resultado[i+1][j+1]=0
                else:
                    resultado[i][j]=255
