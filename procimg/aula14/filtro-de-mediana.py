...
def convolucionar(img, mascara):
  ...
  for i in range(margemInicio, img.shape[0] - margemFinal):
    for j in range(margemInicio, img.shape[1] - margemFinal):
      ...
      cinzaConvolucionada[i, j] = numpy.median(arrayPixels)
...
imgConvolucionada = convolucionar(cinza, masc(3))
cv2.imshow("Convolucionada Mediana", resize(imgConvolucionada))
