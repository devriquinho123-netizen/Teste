...
def fitQtd(qtd, img, mascara, procurandoPreto = False):
  imgRecursiva = fit(img, mascara, procurandoPreto)
  for i in range(qtd - 1):
    imgRecursiva = fit(imgRecursiva, mascara, procurandoPreto)
  return imgRecursiva

def subtrair(img1, img2, procurandoPreto = False):
...
imgErodida = fitQtd(1, pretoBranco, masc(3), False)
cv2.imshow("Erodida", resize(imgErodida))

imgSubtraida = subtrair(pretoBranco, imgErodida, False)
cv2.imshow("Contornos", resize(imgSubtraida))
